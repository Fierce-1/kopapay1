...
import {View, StyleSheet, TouchableOpacity, Text, Alert, Dimensions, ImageBackground, ScrollView} from 'react-native'
...
function Home({navigation}){
    ...
    const renderButtonLoan = (item, index) => {
        ...
        const isLastSelection = index === buttons.length - 1;
        return (
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
                {!isFirstSelection && (
                    <Text style={{ color: 'black', marginEnd: '10%', fontSize: 12 }}>
                    {'<<'} Slide Left
                    </Text>
                )}
                {isFirstSelection && (
                    <View style={{ color: 'white', width: '27%'}}>
            ...
            {!isLastSelection && (
                <Text style={{ color: 'black', marginStart:'10%', fontSize: 12 }}>
                Slide Right {'>>'}
                </Text>
            )}    
            {isLastSelection && (
                    <View style={{ color: 'white', width: '30%'}}>
    ...
    const scale = position === 0 ? 1 : 0.8;
    const opacity = position === 0 ? 1 : 0.6;
    const isFirstSelection = indexDay === 0;
    const isLastSelection = indexDay === buttonsDay.length - 1;
    return (
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
            {!isFirstSelection && (
                <Text style={{ color: 'black', marginEnd: '10%', fontSize: 12 }}>
        ...
        <View style={styles.container}>
            ...
            <ImageBackground source={image} resizeMode="cover" style={styles.image}>
                <View style={{flexDirection: 'row'}}>
                    ...
                    <App_icon imgStyle={{marginTop: '2%', marginStart: '30%', height: windowHeight * 0.1, width: windowWidth * 0.2}} img={logo}/>
                </View>
                <App_text textStyle={styles.txt('white', '5%', '23%', 18, 'bold')} text="Thank you for choosing us."/>
                <App_text textStyle={styles.txt('white', 0, '23%', 11, 'light')} text = "Easy loans, Instant loans, Reliable lenders."/>
